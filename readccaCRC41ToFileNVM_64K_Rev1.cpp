...
//			gpio_set_value_spi(SS6, LOW);
			...
			message1[1].cs_change = 0;//1;							//release the chip select line
			usleep(500);
//			gpio_set_value_spi(SS2, LOW);
//			gpio_set_value_spi(SS0, LOW);
//			gpio_set_value_spi(SS4, LOW);
...
//////////////////////read cca pages and dump to a file
char  debugFilePath[]="/root/PROGRAM/CLemFile/TEST_files/CCA_nvm_64k";
void printCCA_nvm_File(int newline,unsigned char * buff,unsigned int pageNUMBER,int crc)//,int len)
{
int i=0;
/*
unsigned char x[256];
...
///fromIndex=atoi(argv[2]);
int newl=atoi(argv[1]);
FILE *fPtr;
fPtr = fopen(debugFilePath, "a");
fPtr=freopen(NULL,"w",fPtr);
fclose(fPtr);
sleep(1);
 system("CLEAR_CCA_NVM_64K_FILE");
sleep(0.5);
...
//if(SearchForData(rx, 256))
{
/*
printf("(HEX DUMP)=> %.2X %.2X %.3X\n",addressmid,fromIndex,toIndex);
*/
/*
			  for (ret = 0; ret <256; ret++) {         //prints returned array on the screen
			      if (!(ret % 16))
			         puts("");
			      printf("%.2X", rx[ret]);
			   }
 ...
 printCCA_nvm_File(newl,rx,addressmid,yeed);
			usleep(1000);
		}
